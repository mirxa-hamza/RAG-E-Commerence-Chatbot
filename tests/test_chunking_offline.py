"""
Offline checks for the semantic chunker. No model download, no network, no API key.
...
rather than a vibe. Whether the chunker helps on a real corpus is a MEASUREMENT, and that
is what eval/run_eval.py is for - mixing the two gives you neither.
...
# Set before importing config: it is read at import time.
...
check("a short vector list falls back to a plain pack instead of misaligning",
...
# ---------------------------------------------------------------- entry point + cache
print("\nchunk_pages() - the single entry point")

check("the only strategy reports itself as semantic",
      chunking.strategy_name() == "semantic", chunking.strategy_name())
...
original_embed = None
try:
    from src.ml import embeddings as _embeddings
    original_embed = _embeddings.embed_passages
    _embeddings.embed_passages = counting_embed

    chunking.clear_cache()
    first_pass = chunking.chunk_pages(doc)
    check("chunk_pages() produces semantic chunks", len(first_pass) >= 1, len(first_pass))
    check("...using the embedder exactly once", calls["n"] == 1, calls["n"])

    second_pass = chunking.chunk_pages(doc)
    check("an identical document is served from the memo cache, not re-embedded",
          calls["n"] == 1, calls["n"])
    check("...and returns the identical chunk list", first_pass == second_pass)

    other = pages_from("\n\n".join(sentence("T3", n) for n in range(4)))
    chunking.chunk_pages(other)
    check("a different document is a cache MISS and does embed", calls["n"] == 2, calls["n"])
    ...
    if original_embed is not None:
        _embeddings.embed_passages = original_embed
    chunking.clear_cache()
