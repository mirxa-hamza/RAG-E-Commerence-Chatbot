...
/* ─────────────────────────── Upload ─────────────────────────────────
   The browser sends PDFs straight to POST /upload, which writes them into the server's
   data folder and starts the same background ingestion job the sync button uses.

   There are two progress phases, because they fail differently and take wildly different
   amounts of time: the transfer (XHR, has real byte counts) and the indexing (polled
   through GET /ingest/status).
   ==================================================================== */
...
async function pollUntilDone() {
  ...
  while (polling) {
    let job;
    try {
      // The background thread does the work; this only reads its progress.
