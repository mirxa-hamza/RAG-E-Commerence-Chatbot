...
Every route here requires a signed-in user, and every document belongs to the account that
uploaded it.
"""
...
def start_ingest(user: dict = Depends(get_current_user)):
    ...
    The scan itself is global - it is the whole data folder being reconciled, not one
    user's documents - but each file is stored under the owner its path names, and the
    status this returns is filtered to the caller.
    """
    uid = user_id_of(user)
    ingestion.start_job()
