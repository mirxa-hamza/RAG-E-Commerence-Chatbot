...
every document is owned by the account that uploaded it. Documents are only ever indexed
from the data folder - /upload writes the file there first, under data/users/<user_id>/,
and the ordinary ingestion job picks it up.

Everything except answer generation runs on this machine: chunking, embedding, the vector
store and re-ranking.
...
from src.ml import embeddings, llm, reranker
from src.services import database, ingestion, sessions, vectorstore
...
def _warm_up() -> None:
    """
    Everything the first QUESTION would otherwise pay for, paid at startup instead.

    Three lazy singletons sit on the retrieval path, and each one loads on first use. That
    is right - loading them at import keeps the port closed and the browser gets
    ERR_CONNECTION_REFUSED - but "not at import" does not have to mean "during someone's
    question". Measured on one cold first question: ~18s for the embedding model and ~15s
    for the cross-encoder.

    Each is warmed independently and failure is swallowed per stage: a warm-up is an
    optimisation, and one failure must not stop the other two from loading, nor take the
    server down. Whatever fails here simply loads lazily as it always did.
    """
    for label, warm in (("embeddings", embeddings.warm_up),
                        ("re-ranker", reranker.warm_up),
                        ("vector store", vectorstore.warm_up)):
        try:
            warm()
        except Exception:
            log.exception("%s warm-up failed; it will load on first use instead.", label)
...
async def lifespan(app: FastAPI):
    ...
    # Say out loud which configuration is actually running. Every one of these is a setting
    # whose wrong value produces a plausible-looking result rather than an error - an
    # ANSWER_STYLE that was edited but never reloaded, say (the prompt is assembled at
    # IMPORT time, so a .env change needs a restart, and without a line like this there is
    # nothing to check that against).
    log.info(
        "Configuration: chunking=semantic (local), embeddings=%s (local), "
        "vectors=%s (local), llm=%s, answers=%s (<=%d words, <=%d bullets)",
        embeddings.provider_name(), vectorstore.backend_name(), LLM_PROVIDER,
        ANSWER_STYLE, ANSWER_MAX_WORDS, ANSWER_MAX_BULLETS,
    )
    if not llm.is_configured():
        log.warning(
            "No LLM credential for LLM_PROVIDER=%s - retrieval works, but questions will "
            "come back asking for a key. See .env.example.", LLM_PROVIDER,
        )
    # A repeated key in .env is kept silently by python-dotenv, with the last one winning.
...
app = FastAPI(
    title="Marginalia",
    description="A local-first, from-scratch RAG API over the PDFs in the data folder.",
    version="4.0",
    lifespan=lifespan,
)
