...
def redact_uri(uri: str) -> str:
    ...
    A connection string is a CREDENTIAL, and this one used to be printed in full in three
    places: two startup log lines and CANNOT_REACH, which the sign-in screen renders
    verbatim to anyone who loads the page while the database is down. So an Atlas password
    was reachable from terminal scrollback, from any log file or log aggregator the app
    feeds, from a screenshot of a failed startup, and from the login screen of a
    misconfigured deployment.

    The host and username are kept: the whole point of these messages is to say WHICH
    database could not be reached, and redacting that would trade one problem for another.
    A URI with no password (a plain local `mongodb://localhost:27017`) is returned unchanged.
    """
    match = re.match(r"^(?P<scheme>[A-Za-z][A-Za-z0-9+.\-]*://)"
                     r"(?P<user>[^:/?#@]+):(?P<password>[^@]*)@(?P<rest>.*)$", uri)
    if not match:
        return uri
    return f"{match.group('scheme')}{match.group('user')}:***@{match.group('rest')}"


SAFE_MONGO_URI = redact_uri(MONGO_URI)

CANNOT_REACH = (
    f"Can't reach MongoDB at {SAFE_MONGO_URI}. Start it and try again - on Windows: "
...
def get_client():
    ...
    if _client is None:
        ...
        log.info("Connecting to MongoDB at %s (database '%s')", SAFE_MONGO_URI, MONGO_DB)
