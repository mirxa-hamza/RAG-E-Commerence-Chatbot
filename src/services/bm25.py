...
* Cost: a single shared index is invalidated by ANY user's upload, and the next question
  ...
  0.09s at 3k chunks, 0.52s at 20k, 2.95s at 60k. Per-user indices mean one person's
  upload only costs that person.
...
def enabled() -> bool:
    ...
    BM25 lives in this process, so building its index means reading the WHOLE corpus once
    per cached index - a fast disk read against the local Chroma folder. Turning it off
    leaves dense search plus the re-ranker, which is the majority of the quality.
    """
    from src.core.config import KEYWORD_SEARCH

    return KEYWORD_SEARCH == "on"
...
def _build(user_id: str) -> Tuple[Optional[BM25Okapi], List[Dict]]:
    ...
    try:
        ...
        # A store read that fails must not take the question down with it: worse ranking
        # beats no answer.
