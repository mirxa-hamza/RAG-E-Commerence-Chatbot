"""
Step 1 of the pipeline: turn a PDF into a stream of sentences, each tagged with the page it
came from (so answers can cite pages). The chunker in services/chunking.py groups them.

The thing that matters here, and that used to be wrong: paragraph structure is preserved
through extraction. Collapsing all whitespace to single spaces (the obvious
`" ".join(text.split())`) destroys every paragraph boundary before anything downstream can
use it, which welds a heading onto unrelated prose and starts sentences mid-thought.
"""
import re
