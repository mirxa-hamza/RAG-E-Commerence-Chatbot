...
def _expand_neighbors(chunks: List[Dict], radius: int,
    ...
    try:
        ...
        # relevance floor, not the hit itself - losing it to a transient store error
        # should degrade the answer, not fail the question. Same trade as a keyword-index
        # build failure: worse context beats no answer.
    ...
    with timed(log, "retrieval"):
        ...
        if use_rerank:
            ...
            if ranked is not None:
                ...
                if not kept:
                    log.info(
                        "No candidate cleared the re-ranker's floor of %s (best was %.2f) "
        ...
        # Semantic chunks do not overlap, by design - the boundary is the point. Neighbour
        # expansion is what stops the model reading one side of a boundary in isolation.
